    ...
    def set_default_params_tools(self, tools, parsed_config, parsed_execution_config, agent_id, model_api_key, session,
        ...
        for tool in tools:
            ...
            if hasattr(tool, 'llm') and (parsed_config["model"] == "gpt4" or parsed_config[
                "model"] == "gpt-3.5-turbo" or parsed_config["model"] == "gpt-4-32k") and tool.name != "QueryResource":
                tool.llm = OpenAi(model="gpt-3.5-turbo", api_key=model_api_key, temperature=0.4) if parsed_config["model"] != "gpt-4-32k" else OpenAi(model="gpt-4-32k", api_key=model_api_key, temperature=0.3)
