...
def build_action_based_agents():
    agent_workflow = session.query(AgentWorkflow).filter(AgentWorkflow.name == "Task Queue Action Based").first()
    if agent_workflow is None:
        agent_workflow = AgentWorkflow(name="Task Queue Action Based", description="Task Queue Action Based")
        session.add(agent_workflow)
        session.commit()
